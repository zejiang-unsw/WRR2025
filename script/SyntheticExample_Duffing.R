...
#ts.r <- WASP2.0::data.gen.Duffing(nobs = n, s=0, do.plot=T)
#plot(ts.r$x, ts.r$y, type="l")

data.list <- NULL
for(r in 1:ensemble){

  ts.r <- WASP2.0::data.gen.Duffing(nobs = n, s=noise, do.plot=FALSE)
  
  data.list[[r]]  <- list(x=ts.r$y, dp=ts.r$x %>% as.matrix())
...
file_dat <- paste0("../result/Duffing_",wf,"_n",n,"_sd",noise,
...
if(!file.exists(file_dat)) {
  ...
  #dwt1 <- dwt.wasp(data.list[[j]], wf, index_c = ind_cal, phase=TRUE, k=5, verbose = F)
  if(flag.v!="_v1"){
    dwt1 <- WASP2.0::dwt.wasp.val(data.list[[j]], wf, index_c = ind_cal, phase=TRUE,
                                  k=n/2-1, max_iter=3, verbose = F)
  } else {
    dwt1 <- WASP2.0::dwt.wasp.val(data.list[[j]], wf, index_c = ind_cal, phase=TRUE,
...
# filen <- paste0("Figure_Duffing_",wf,"_n",n,"_sd",noise,"_r",ensemble,
#                 flag.v,".png")
