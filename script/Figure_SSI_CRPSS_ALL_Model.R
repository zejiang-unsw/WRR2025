...
## fig-box ----
RPSS_df_all1 <- RPSS_df %>% gather(Model, RPSS, 4:6) %>% subset(RPSS > 0)
#RPSS_df_all1 <- RPSS_df_all %>% subset(RPSS > 0)
#RPSS_df_all1 <- RPSS_df_all 
summary(RPSS_df_all1)

RPSS_df_all1$CI <- factor(RPSS_df_all1$CI, levels=CI_names)
RPSS_df_all1$Model <- factor(RPSS_df_all1$Model, levels=c("Mod3","Mod2","Mod1","Mod0"))
RPSS_df_all1$Lead <- factor(RPSS_df_all1$Lead, labels = paste0("Lead ",0:2))

labels_mod_exp1 <- c(expression(mu~" "), expression(mu(x)), expression(mu("x'")),
                     expression(mu("x'(φ)")))[c(4,3,2,1)]
...
# Step 2: plot
p_box <- ggplot(data = RPSS_summary, aes(x = CI, y = mean_RPSS, color = Model)) +
  ...
  scale_color_manual(values = col_qual[c(4, 3, 2)], labels = labels_mod_exp1) +
...
### Figure 5a----
if(flag.save){
  filen <- paste0("Figure_CRPSS_ALL_",mode,"_",wf,"_",method_fevd,flag.v,"_AU_SSI_",flag.var,".png")
